...
public class FxmlNavigator_Test {
    ...
    void openInNewWindow_ioException_returnsNull() throws Exception {
        ...
        try (MockedStatic<SpringFXMLLoader> st = mockStatic(SpringFXMLLoader.class); MockedConstruction<Stage> stg = mockConstruction(Stage.class); MockedConstruction<Scene> sc = mockConstruction(Scene.class)) {st.when(() -> SpringFXMLLoader.load("/error.fxml")).thenReturn(loader);Object result = createNavigator().openInNewWindow("/error.fxml", "Error");assertNull(result);
        }
    }
}
