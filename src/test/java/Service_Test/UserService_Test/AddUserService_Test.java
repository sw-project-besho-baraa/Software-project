...
public class AddUserService_Test {
    ...
    void addUser_hashesPasswordAndSavesUser() {
        ...
        verify(userRepository, times(1)).save(argThat(user -> user.getName().equals(name) && user.getEmail().equals(email) && user.getHashedPassword().equals(hashedPassword) && user.getUserRole() == role
        ));
    }
